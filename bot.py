...
# Système de prime (uniquement pour les pirates)
BOUNTY_START = 10_000_000
BOUNTY_MAX_AT_TOP_RANK = 1_400_000_000  # Plafond pour Commandant de Yonko (le vrai Yonko, à 2.3 Md, s'obtient par sub, hors bot)

def compute_bounty(xp):
    max_xp = XP_THRESHOLDS[-1]
    progress = min(xp, max_xp) / max_xp
    bounty = BOUNTY_START + progress * (BOUNTY_MAX_AT_TOP_RANK - BOUNTY_START)
    return int(bounty)
...
async def profil_cmd(interaction: discord.Interaction):
    ...
    if branch == "pirate":
        bounty = compute_bounty(xp)
        bounty_str = f"{bounty:,}".replace(",", " ")
        embed.add_field(name="💰 Prime", value=f"{bounty_str} Berry", inline=False)
...
async def classement_cmd(interaction: discord.Interaction, branche: app_commands.Choice[str] = None):
    ...
    for i, (uid, entry) in enumerate(top, start=1):
        ...
        if entry["branch"] == "pirate":
            bounty = compute_bounty(entry["xp"])
            bounty_str = f"{bounty:,}".replace(",", " ")
            lines.append(f"**{i}.** {icon} {name} — {role_name} — {bounty_str} Berry ({entry['xp']} XP)")
        else:
